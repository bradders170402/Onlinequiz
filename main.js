...
const renderQuestion = () => {
    ...
    nextButton.textContent = currentQuestion === questionArray.length - 1 ? "Submit" : "Next";
};
...
const checkAnswer = () => {
    ...
    if (questionArray[currentQuestion].correctAnswer === parseInt(selectedOption.value)) {
        score++;
    }
    return true;
};
...
document.getElementById("next-btn").addEventListener("click", () => {
    ...
    if (currentQuestion < questionArray.length - 1) {
        ...
        alert(`Quiz submitted! Your final score is: ${score}/${questionArray.length}`);
        ...
        restartBtn.style.display = "inline-block"; // Fallback to ensure visibility
...
document.getElementById("restart-btn").addEventListener("click", () => {
    score = 0;
    currentQuestion = 0;
    selectedAnswers = new Array(questionArray.length).fill(null);
